...
int kmerExpRunner::extractByCountingUniqKmersFromSetsIterative(const njh::progutils::CmdArgs & inputCommands){
	uint32_t numThreads = 1;
	...
	{//initial
		...
		OutOptions outCountsOpts(njh::files::make_path(setUp.pars_.directoryName_, "initialExtractionCounts.tab.txt"));
	...
	while(iterate){
		...
		for (const auto &kmerSet: uniqueKmersPerSet) {
			...
			if (inOpts.inExists()) {
				...
				while (extractedReader.readNextRead(pseq)) {
					for (uint32_t pos = 0; pos < len(pseq.seqBase_.seq_) - klen + 1; ++pos) {
						rawKmersPerInput[kmerSet.first].emplace(
										hasher.hash(pseq.seqBase_.seq_.substr(pos, klen)));
					}
					for (uint32_t pos = 0; pos < len(pseq.mateSeqBase_.seq_) - klen + 1; ++pos) {
						rawKmersPerInput[kmerSet.first].emplace(
										hasher.hash(pseq.mateSeqBase_.seq_.substr(pos, klen)));
					}
				}
			}
		}
		...
		for (const auto & finalNewKmerSet : rawKmersPerInput){
			for (const auto finalNewKmer: finalNewKmerSet.second) {
				...
				if (pass) {
					outputUniqueKmersPerSet[finalNewKmerSet.first].emplace(finalNewKmer);
				}
			}
		}
		...
		OutOptions outCountsOpts(njh::files::make_path(setUp.pars_.directoryName_, njh::pasteAsStr(iterNumber, "_ExtractionCounts.tab.txt")));
...
//		readInComp = [&reader, &uniqueKmersPerSet, &uniqueKmersFoundPerSet,&kmersFoundPerSeq,&mut,&klen,&includeRevComp]() {
	...
	if(extractAfterIterating){
		...
		OutOptions outCountsOpts(njh::files::make_path(finalExtractionDir, "extractionCounts.tab.txt"));
