//
// Created by Nicholas Hathaway on 9/12/24.
//
...
class OtherAssemblersUtilityForUniqueKmerExtraction{
public:
	struct InputPars{
		std::string regionUid_;
		bfs::path extractionOutputDir_;
		std::string sample_;
		bfs::path bedFile_;
		bfs::path twobitFnp_;
	...
	explicit OtherAssemblersUtilityForUniqueKmerExtraction(InputPars pars): inputPars_(std::move(pars)){
		...
		singlesFnp_ =  njh::files::make_path(finalExtractionDir_, inputPars_.regionUid_ + ".fastq.gz");
		{
		...
		outputFnp_ = njh::files::make_path(inputPars_.outputDir_, "output.fasta");
		outputAboveCutOffFnp_ = njh::files::make_path(inputPars_.outputDir_, "output_aboveCutOff.fasta");
...
int programWrappersAssembleOnPathWeaverRunner::runMIRAOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
...
//				contigsSeqIoOpts.includeWhiteSpaceInName_ = false;
	...
	OutputStream basicInfo(njh::files::make_path(utility.inputPars_.outputDir_, "basicInfoPerRegion.tab.txt"));
	...
	OutputStream exceptionsOut(njh::files::make_path(utility.inputPars_.outputDir_, "exceptionsMessages.tab.txt"));
	exceptionsOut << "regionUID\tmessage" << std::endl;
	exceptionsOut << utility.inputPars_.regionUid_ << "\t" << exceptionMess << std::endl;
	return 0;
}
//
int programWrappersAssembleOnPathWeaverRunner::runFermiLiteOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
	...
	try {
		...
		if(bfs::exists(utility.pairedR1Fnp_)){
			filesToCollapse.emplace_back(utility.pairedR1Fnp_);
			filesToCollapse.emplace_back(utility.pairedR2Fnp_);
		}
		if(bfs::exists(utility.singlesFnp_)){
			filesToCollapse.emplace_back(utility.singlesFnp_);
		}
...
int programWrappersAssembleOnPathWeaverRunner::runUnicyclerOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
...
int programWrappersAssembleOnPathWeaverRunner::runRayOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
	...
	inPars.setPars(setUp);
	setUp.setOption(RayKmerLength, "--RayKmerLength", "Ray Kmer Length", true);
...
int programWrappersAssembleOnPathWeaverRunner::runIDBAUDOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
	...
	inPars.setPars(setUp);
	setUp.setOption(IDBAUDKmerLength, "--IDBAUDKmerLength", "IDBAUD Kmer Length", true);
...
int programWrappersAssembleOnPathWeaverRunner::runTrinityOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
...
int programWrappersAssembleOnPathWeaverRunner::runMegahitOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
...
int programWrappersAssembleOnPathWeaverRunner::runSavageOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
...
int programWrappersAssembleOnPathWeaverRunner::runPolyteOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
	bfs::path haploconductPath = "/home/hathawan/sourceCodes/savage/HaploConduct/haploconduct";
	...
	setUp.setOption(possibleAvgCoverage, "--possibleAvgCoverage", "possibleAvgCoverage", true);
	setUp.setOption(insertSizeSD, "--insertSizeSD", "insertSizeSD", true);
	setUp.setOption(insertSize, "--insertSize", "insertSize", true);
...
int programWrappersAssembleOnPathWeaverRunner::runPRICEOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
	uint32_t insertSize = 0;
...
int programWrappersAssembleOnPathWeaverRunner::runVelvetOptimizerAndMetaVelvetOnUniqueKmerExtraction(const njh::progutils::CmdArgs & inputCommands) {
...
//	}
	...
	OtherAssemblersUtilityForUniqueKmerExtraction utilityMetaVelvet(inParsMetaVelvet);
...
//					contigsSeqIoOpts.includeWhiteSpaceInName_ = false;
...

	{
		...
		OutputStream basicInfo(njh::files::make_path(utilityMetaVelvet.inputPars_.outputDir_, "basicInfoPerRegion.tab.txt"));
		...
		OutputStream exceptionsOut(njh::files::make_path(utilityMetaVelvet.inputPars_.outputDir_, "exceptionsMessages.tab.txt"));
		exceptionsOut << "regionUID\tmessage" << std::endl;
