...
int seqUtilsExtractRunner::getSimilarSequencesByKDist(const njh::progutils::CmdArgs & inputCommands) {
	seqUtilsExtractSetUp setUp(inputCommands);
...
int seqUtilsExtractRunner::getSimilarSequences(const njh::progutils::CmdArgs & inputCommands) {
  ...
  setUp.setUpGetSimilarSequences(useNucComp, maxNucCompDiff, idCutOff, gapCutoff, queryCutoff);
  setUp.startARunLog(setUp.pars_.directoryName_);
  SeqInput reader(setUp.pars_.ioOptions_);
  	reader.openIn();
	auto inReads = reader.readAllReads<readObject>();
