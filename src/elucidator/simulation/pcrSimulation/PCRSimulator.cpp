...
Json::Value ChimeraProfiler::toJson() const {
	Json::Value ret;
	ret["class"] = njh::json::toJson(njh::getTypeName(*this));
	ret["segsForSeqs_"] = njh::json::toJson(segsForSeqs_);
	return ret;
}


ChimeraProfiler::SharedSegments::Segment::Segment(const uint32_t starta, const uint32_t startb,
                                                  uint32_t size) : starta_(starta), startb_(startb), size_(size) {
}

uint32_t ChimeraProfiler::SharedSegments::Segment::enda() const {
	return starta_ + size_;
}

uint32_t ChimeraProfiler::SharedSegments::Segment::endb() const {
	return startb_ + size_;
}

bool ChimeraProfiler::SharedSegments::Segment::doesSegsOverLapInA(const Segment& otherSeg) {
	if ((otherSeg.starta_ > starta_ && otherSeg.starta_ < enda()) || (
		    otherSeg.enda() > starta_ && otherSeg.enda() < enda())) {
		return true;
	}
	return false;
}

bool ChimeraProfiler::SharedSegments::Segment::doesSegsOverLapInB(const Segment& otherSeg) {
	if ((otherSeg.startb_ > startb_ && otherSeg.startb_ < endb()) || (
		    otherSeg.endb() > startb_ && otherSeg.endb() < endb())) {
		return true;
	}
	return false;
}

Json::Value ChimeraProfiler::SharedSegments::Segment::toJson() const {
	Json::Value ret;
	ret["class"] = njh::json::toJson(njh::getTypeName(*this));

	ret["starta_"] = njh::json::toJson(starta_);
	ret["startb_"] = njh::json::toJson(startb_);
	ret["size_"] = njh::json::toJson(size_);
	return ret;
}


ChimeraProfiler::SharedSegments::SharedSegments::SharedSegments(const std::string& namea,
                                                                const std::string& nameb) : seqNameA_(namea),
	seqNameB_(nameb) {
}

void ChimeraProfiler::SharedSegments::SharedSegments::addSegment(Segment newSeg) {
...
void ChimeraProfiler::SharedSegments::SharedSegments::addSegment(uint32_t starta, uint32_t startb, uint32_t size) {
	addSegment(Segment{starta, startb, size});
}
Json::Value ChimeraProfiler::SharedSegments::SharedSegments::toJson() const {
	Json::Value ret;
	ret["class"] = njh::json::toJson(njh::getTypeName(*this));

	ret["seqNameA_"] = njh::json::toJson(seqNameA_);
	ret["seqNameB_"] = njh::json::toJson(seqNameB_);
	ret["segs_"] = njh::json::toJson(segs_);
	return ret;
}

ChimeraProfiler::SegmentsForSeqs::SegmentsForSeqs(const seqInfo& seqBase) : seqBase_(seqBase) {
}

uint32_t ChimeraProfiler::SegmentsForSeqs::minSegmentStart() {
...
uint32_t ChimeraProfiler::SegmentsForSeqs::maxSegmentEnd() {
	uint32_t ret = 0;
	for (const auto& seg: sharedSegs_) {
...
void ChimeraProfiler::SegmentsForSeqs::addSegments(const seqInfo& alnA, const seqInfo& alnB, uint32_t padding) {
	...
	uint32_t lastNonGap = std::min(alnA.seq_.find_last_not_of('-'), alnB.seq_.find_last_not_of('-'));
	...
	// std::cout << "alnA.name_: " << alnA.name_ << std::endl;
	// std::cout << "alnB.name_: " << alnB.name_ << std::endl;
	// std::cout << "firstNonGap: " << firstNonGap << std::endl;
	// std::cout << "lastNonGap: " << lastNonGap << std::endl;
	//
	// std::cout << std::endl;

	for (const auto pos: iter::range(firstNonGap, lastNonGap + 1)) {
		// if (alnA.seq_[pos] != '-' && alnA.seq_[pos] == alnB.seq_[pos]) {
		if (alnA.seq_[pos] == alnB.seq_[pos]) {
			...
			if (currentSegLength > 0 && currentSegLength > padding) {
				...
				//						std::cout << std::endl;
				if (!(startA + currentSegLength == aLen && startB + currentSegLength == bLen) && !(
					    0 == startA && 0 == startB)) {
...
ChimeraProfiler::SegmentsForSeqs::SeqNameSeg::SeqNameSeg(const std::string& name,
                                                         uint32_t pos): name_(name), pos_(pos) {
}

Json::Value ChimeraProfiler::SegmentsForSeqs::SeqNameSeg::toJson() const {
	Json::Value ret;
	ret["class"] = njh::json::toJson(njh::getTypeName(*this));
...
std::vector<ChimeraProfiler::SegmentsForSeqs::SeqNameSeg>
ChimeraProfiler::SegmentsForSeqs::genPosChimeras(uint32_t pos) {
...
Json::Value ChimeraProfiler::SegmentsForSeqs::toJson() const {
	Json::Value ret;
	ret["class"] = njh::json::toJson(njh::getTypeName(*this));

	ret["seqBase_"] = njh::json::toJson(seqBase_);
	ret["sharedSegs_"] = njh::json::toJson(sharedSegs_);
	return ret;
}


PCRSimulator::PCRSimulator(uint64_t intErrorRate) : intErrorRate_(intErrorRate) {
}
...
PCRSimulator::PCRProduct::PCRProduct(const std::string& pcrSeq,
                                     const uint32_t pcrRoundsLeft, const uint32_t templateAmount) : pcrSeq_(pcrSeq),
	pcrRoundsLeft_(pcrRoundsLeft), templateAmount_(
		templateAmount) {
}

PCRSimulator::PCRProduct::PCRProduct(const std::string& pcrSeq,
                                     const uint32_t pcrRoundsLeft) : pcrSeq_(pcrSeq), pcrRoundsLeft_(pcrRoundsLeft) {
}
...
void PCRSimulator::runPcr(uint32_t numThreads,
	...
	for (uint32_t t = 0; t < numThreads; ++t) {
		...
		std::unordered_map<char, njh::randObjectGen<char, uint32_t>> charGen;
		charGen.emplace(
			'T', njh::randObjectGen<char, uint32_t>(std::vector<char>{'A', 'C', 'G'}, std::vector<uint32_t>{1, 80, 1}));
		charGen.emplace(
			'C', njh::randObjectGen<char, uint32_t>(std::vector<char>{'A', 'T', 'G'}, std::vector<uint32_t>{1, 80, 1}));
		charGen.emplace(
			'G', njh::randObjectGen<char, uint32_t>(std::vector<char>{'C', 'A', 'T'}, std::vector<uint32_t>{1, 80, 1}));
		charGen.emplace(
			'A', njh::randObjectGen<char, uint32_t>(std::vector<char>{'C', 'G', 'T'}, std::vector<uint32_t>{1, 80, 1}));
	...
	pcrProducts.emplace(currentSeq, pcrRounds, currentStartingTemplate);
	while (!pcrProducts.empty()) {
		...
		for (uint32_t round = 1; round <= currentPCRProduct.pcrRoundsLeft_; ++round) {
			...
			if (duplicatingAmount > 0) {
				...
				auto mutate = [this, &readPositions,&mutsLock,&muts,&tempAmounts,&currentPCRProduct,&gens,&charGens,&
							numberMutated,&mutants](uint32_t threadNum) {
					if (tempAmounts[threadNum] > 0) {
						...
						for (uint32_t read = 0; read < tempAmounts[threadNum]; ++read) {
							...
							if (count != 0) {
								++currentMuts[count];
								currentMutSeqs.emplace_back(mutateSeq(currentPCRProduct.pcrSeq_, count, readPositions, gens[threadNum],
								                                      charGens[threadNum]));
							}
						}
...
PCRSimulator::SeqGenomeCnt::SeqGenomeCnt(const seqInfo& seqBase,
                                         const uint64_t genomeCnt) : seqBase_(seqBase), genomeCnt_(genomeCnt) {
}
...
PCRSimulator::SimHapCounts PCRSimulator::simLibFast(const std::vector<SeqGenomeCnt>& seqs,
		...
		if (numberOfSeqsSampled > 1) {
			...
			if (seqsWithSegs > 0) {
				...
				std::mutex chimerasMut; {
					table roundCountsTab(VecStr{
						"round", "seqName", "total", "fullTemplateFinished", "fullTemplateThisRound", "partialThisRound",
						"fracPartialThisRound", "partialTotalAmount", "partialTotalAmountFrac"
					});
					...
					for (auto round: iter::range<uint32_t>(1, initialPcrRounds + 1)) {
						...
						for (const auto& seqSampled: seqsSampled) {
							...
							uint64_t partial = 0;
							if (round > 10) {
								finished += std::min<uint64_t>(std::round(pcrEfficiency_ * finishedAmount[seqSampled.seqBase_.name_]),
								                               templateCap_);
								partial += std::min<uint64_t>(
									std::round((1 - pcrEfficiency_) * finishedAmount[seqSampled.seqBase_.name_]), templateCap_);
							} else {
							...
							if (partial > 0) {
								...
								auto genPartials = [&partials,&seqSampled,&chiProf,&chiStart,&chiStop,&chimerasMut,&chimeras,&
							...
							roundCountsTab.addRow(round, seqSampled.seqBase_.name_,
							                      partialTotalAmount[seqSampled.seqBase_.name_] + finishedAmount[seqSampled.seqBase_.
								                      name_], finishedAmount[seqSampled.seqBase_.name_],
							                      finished, partial, static_cast<double>(partial) / (finished + partial),
							                      partialTotalAmount[seqSampled.seqBase_.name_],
							                      static_cast<double>(partialTotalAmount[seqSampled.seqBase_.name_]) / (
								                      partialTotalAmount[seqSampled.seqBase_.name_] + finishedAmount[seqSampled.seqBase_
									                      .name_]));
						}
						...
						for (const auto& chiSeq: chimeraSeqs) {
							...
							if (round > 10) {
								...
								partial += std::min<uint64_t>(std::round((1 - pcrEfficiency_) * finishedAmount[chiSeq.name_]),
								                              templateCap_);
							} else {
							...
							if (partial > 0) {
								...
								auto genPartials = [&partials,&chiSeq,&chiProf,&chiStart,&chiStop,&chimerasMut,&chimeras,&finishedAmount
							...
							roundCountsTab.addRow(round, chiSeq.name_,
							                      partialTotalAmount[chiSeq.name_] + finishedAmount[chiSeq.name_],
							                      finishedAmount[chiSeq.name_],
							                      finished, partial, static_cast<double>(partial) / (finished + partial),
							                      partialTotalAmount[chiSeq.name_],
							                      static_cast<double>(partialTotalAmount[chiSeq.name_]) / (
								                      partialTotalAmount[chiSeq.name_] + finishedAmount[chiSeq.name_]));
						}
						//add in chimeras
						for (const auto& chi: chimeras) {
							...
							if (!matchesInputSeq && !matchesPreviousChimera) {
								...
								maxLen = readVec::getMaxLength(chimeraSeqs);
					...
					for (const auto& seqSampled: seqsSampled) {
						allFinishedPlusPartial += finishedAmount[seqSampled.seqBase_.name_] + partialTotalAmount[seqSampled.seqBase_
							.name_];
						percPartial[seqSampled.seqBase_.name_] =
								static_cast<double>(partialTotalAmount[seqSampled.seqBase_.name_]) / (
									partialTotalAmount[seqSampled.seqBase_.name_] + finishedAmount[seqSampled.seqBase_.name_]);
					}
					VecStr seqNames;
					std::vector<double> seqPercs;
					for (const auto& seqSampled: seqsSampled) {
						double perc = static_cast<double>(finishedAmount[seqSampled.seqBase_.name_] + partialTotalAmount[seqSampled.
							                                  seqBase_.name_]) / allFinishedPlusPartial;
					...
					for (uint32_t r = 0; r < finalReadAmount - (
						                     finalReadAmount * (
							                     static_cast<double>(intialRoundChiTotal) / (
								                     all_intialFinishedAmount + intialRoundChiTotal))); ++r) {
					...
					for (const auto& finalPartial: finalPartials) {
						...
						auto genPartials = [&finalPartial,&partials,&chiProf,&chiStart,&chiStop,&chimerasMut,&finishedAmount,&
							...
							for (uint64_t temp = 0; temp < partials[threadNum]; ++temp) {
								...
								auto partLength = ranGen.unifRand<uint32_t>(
									0, chiProf.segsForSeqs_[finalPartial.first]->seqBase_.seq_.size());
								//											std::cout << "chiStart: " << chiStart  << " chiStop:" << chiStop << std::endl;
								//											std::cout << "partLength: " << partLength << " chi pos? " << njh::colorBool(partLength >= chiStart && partLength <chiStop) << std::endl;
								if (partLength >= chiStart && partLength < chiStop) {
									auto posChis = chiProf.segsForSeqs_[chiProf.segsForSeqs_[finalPartial.first]->seqBase_.name_]->
											genPosChimeras(partLength);
									VecStr names;
					...
					for (const auto& finalChimeraForRound: finalChimerasPerRound) {
						...
						for (const auto& finalChimera: finalChimeraForRound.second) {
							...
							if (!matchesInputSeq) {
								...
								if (!matchesInputSeq && !matchesPreviousChimera) {
									std::string name = njh::pasteAsStr("Chi.", chimeraSeqs.size(), "[PCRRound=",
									                                   finalChimeraForRound.first, "]");
					...
					for (const auto& finalChimeraForRound: finalChimerasPerRound) {
						...
						if (!barcodedSeqsCurrent.empty()) {
							auto sampleNumberAll = sampleReadsWithoutReplacementFinishPCR(barcodedSeqsCurrent,
							                                                              currentChiAllSeqCounts,
							                                                              totalsForRounds[finalChimeraForRound.first],
							                                                              libOutFile, seqFileLock,
							                                                              pcrRounds - finalChimeraForRound.first,
							                                                              numThreads);
	...
	if (verbose_) {
		...
		std::cout << "ideal templateAmountAfterInitialPCR: " << totalTemplateStrandCount * std::pow(2, initialPcrRounds) -
				totalTemplateStrandCount << std::endl;
	}

	//now sample the rest of PCR
	auto sampleNumberAll = sampleReadsWithoutReplacementFinishPCR(barcodedSeqs,
	                                                              allSeqCounts, finalReadAmount - totalFinalChimeraCount,
	                                                              libOutFile, seqFileLock,
...
std::unordered_map<std::string, PCRSimulator::SimHapCounts::MutInfo>
PCRSimulator::sampleReadsWithoutReplacementFinishPCR(
	...
	if (seqNames != multipleSeqCountsNames) {
		std::stringstream ss;
		ss << "Error in " << __PRETTY_FUNCTION__ << " names for seqNames and multipleSeqCountsNames counts don't match" <<
				std::endl;
		...
		for (auto& seqCounts: multipleSeqCounts) {
			...
			for (auto& seqCount: seqCounts.second) {
				...
				if (sum >= randSel) {
					...
					allSampledSeqs[read % numThreads].emplace_back(std::pair<std::string, std::string>{
						seqCounts.first, seqCount.first
					});
					foundSelection = true;
					break;
				}
	...
	auto finishPCR =
			[this,&allSampledSeqs,&numberOfPCRRoundsLeft,&seqs,&seqFileLock,&sequenceOutFile,&ret,&finalReadAmount](
